...
class CreateBuiltInAttribute(bpy.types.Operator):
    """
    Creates attributes that are generated by blender when using some nodes 
    """
    ...
    built_in_attribute_enum: bpy.props.EnumProperty(
        name="Attribute",
        description="Select an option",
        items=func.get_built_in_attributes_enum
    )

    built_in_attribute_domain: bpy.props.EnumProperty(
        name="Domain",
        description="Select an option",
        items=func.get_built_in_attribute_compatible_domains_enum
    )

    built_in_attribute_data_type: bpy.props.EnumProperty(
        name="Data Type",
        description="Select an option",
        items=func.get_built_in_attribute_compatible_data_types_enum
    )

    # Property group with all available attribute data types
    attribute_values_propgroup: bpy.props.PointerProperty(type=func.get_all_attribute_data_types_property_group(False))
    ...
    b_allow_custom_values: bpy.props.BoolProperty(name="Custom Value",
    ...
    def poll(self, context):
        ...
        if not obj:
            ...
            self.poll_message_set("Object type is not supported")
            return False
        elif not func.pinned_mesh_poll(self, context, self.b_pinned_mesh_support):
            return False
        return True
    ...
    def create_built_in_attribute(self, context):
        ...
        new_attr_def = func.get_built_in_attribute(self.built_in_attribute_enum)

        # Set new attribute name
        if self.built_in_attribute_enum in [".vs.", ".es.", ".pn."]:
            uvmaps = func.get_uvmaps(obj_data)
        ...
        if self.b_allow_custom_values:
            value = func.get_all_attribute_datatype_property_group_value(self.attribute_values_propgroup,
                                                                               self.built_in_attribute_data_type)
        else:
            value = new_attr_def.default_value
        func.set_attribute_values(new_attrib, value)
    ...
    def enum_watchdog_attribute(self, context):
        ...
        try:
            if getattr(self, f'built_in_attribute_enum') == '':
                setattr(self, f'built_in_attribute_enum', func.get_built_in_attributes_enum(self, context)[0][0])
        except TypeError:
            pass
    ...
    def enum_watchdog_domain(self, context):
        ...
        try:
            if getattr(self, f'built_in_attribute_domain') == '':
                setattr(self, f'built_in_attribute_domain', func.get_built_in_attribute_compatible_domains_enum(self, context)[0][0])
        except TypeError:
            pass
    ...
    def enum_watchdog_data_type(self, context):
        ...
        try:
            if getattr(self, f'built_in_attribute_data_type') == '':
                setattr(self, f'built_in_attribute_data_type', func.get_built_in_attribute_compatible_data_types_enum(self, context)[0][0])
        except TypeError:
            pass
    ...
    def execute(self, context):
        try:
            ...
            etc.call_catastrophic_crash_handler(CreateBuiltInAttribute, exc)
            return {"CANCELLED"}
    ...
    def draw(self, context):
        ...
        c.separator()
        c.label(text='Domain')
        if len(func.get_built_in_attribute_compatible_domains_enum(self, context)) == 1:
            c.label(text=func.get_friendly_domain_name(self.built_in_attribute_domain), 
                    icon=func.get_domain_icon(self.built_in_attribute_domain))
        else:
        ...
        if len(func.get_built_in_attribute_compatible_data_types_enum(self, context)) == 1:
            c.label(text=func.get_friendly_data_type_name(self.built_in_attribute_data_type), 
                    icon=func.get_data_type_icon(self.built_in_attribute_data_type))
        else:
        ...
        if self.built_in_attribute_enum in obj_data.attributes:
            if self.b_allow_custom_values:
                val_str = str(func.get_all_attribute_datatype_property_group_value(self.attribute_values_propgroup,
                                                                               self.built_in_attribute_data_type))
            else:
        ...
        # Wiki
        etc.append_wiki_operator(self, context)
